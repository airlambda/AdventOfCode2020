...
def check_birth_year(input_string):
    if len(input_string) != 4:
        return False
    elif int(input_string) >= 1920 and int(input_string) <= 2002:
        return True
    else:
        return False

def check_issue_year(input_string):
    if len(input_string) != 4:
        return False
    elif int(input_string) >= 2010 and int(input_string) <= 2020:
        return True
    else:
        return False

def check_expiry_year(input_string):
    if len(input_string) != 4:
        return False
    elif int(input_string) >= 2020 and int(input_string) <= 2030:
        return True
    else:
        return False

def check_height(input_string):
    if input_string[-2:] not in ['cm','in']:
        return False
    elif input_string[-2:] == 'cm':
        if int(input_string[:-2]) >= 150 and int(input_string[:-2]) <= 193:
            return True
    elif input_string[-2:] == 'in':
            if int(input_string[:-2]) >= 59 and int(input_string[:-2]) <= 76:
                return True 
    else:
        return False

def check_hair_colour(input_string):
    if input_string[0] != '#':
        return False
    hair_colour_regex = r'[0-9a-f]+'
    match = re.match(hair_colour_regex, input_string[1:])
    if match is None:
        return False
    elif len(match.string) != 6:
        return False
    else:
        return True

def check_eye_colour(input_string):
    allowable_eye_colours = ['amb','blu','brn','gry','grn','hzl','oth']
    if input_string not in allowable_eye_colours:
        return False
    else:
        return True

def check_passport_id(input_string):
    passport_id_regex = r'[0-9]+'
    match = re.match(passport_id_regex, input_string)
    if match is None:
        return False
    elif len(match.string) != 9:
        return False
    else:
        return True
...
for passport_data in input_split:
    regex_string = r'[a-z]{3}:[a-zA-z0-9#]+'
    field_data = re.findall(regex_string, passport_data)
    
    for matches in field_data:
        field_name = matches[0:3]
        if field_name == 'byr':
            if check_birth_year(matches[4:]):
                mandatory_fields.remove(field_name)
        elif field_name == 'iyr':
            if check_issue_year(matches[4:]):
                mandatory_fields.remove(field_name)
        elif field_name == 'eyr':
            if check_expiry_year(matches[4:]):
                mandatory_fields.remove(field_name)
        elif field_name == 'hgt':
            if check_height(matches[4:]):
                mandatory_fields.remove(field_name)
        elif field_name == 'hcl':
            if check_hair_colour(matches[4:]):
                mandatory_fields.remove(field_name)
        elif field_name == 'ecl':
            if check_eye_colour(matches[4:]):
                mandatory_fields.remove(field_name)
        elif field_name == 'pid':
            if check_passport_id(matches[4:]):
                mandatory_fields.remove(field_name)
    
    # Check for remaining fields in mandatory fields
    if mandatory_fields and all(elem == 'cid' for elem in mandatory_fields) or len(mandatory_fields) == 0:
        valid_passports += 1
