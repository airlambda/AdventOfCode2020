...
REGEX_STRING = r'^(?P<first_position>\d{1,2})[-](?P<second_position>\d{1,2})[\s](?P<letter>[a-z])[:][\s](?P<password>[a-z]*)'
...
for string in input_passwords:
    ...
    if len(string) != 0:
        ...
        first_index = int(match.group('first_position')) - 1
        second_index = int(match.group('second_position')) - 1
        if (match.group('password')[first_index] == match.group('letter'))^(match.group('password')[second_index] == match.group('letter')):
            num_valid += 1
