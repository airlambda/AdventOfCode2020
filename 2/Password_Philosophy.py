...
input = """1-14 b: bbbbbbbbbbbbbbbbbbb
...
REGEX_STRING = r'^(?P<lower_bound>\d{1,2})[-](?P<upper_bound>\d{1,2})[\s](?P<letter>[a-z])[:][\s](?P<password>[a-z]*)'

num_valid = 0

input_passwords = input.split('\n')

for string in input_passwords:
    # Use regexes to split up the string into constituent parts:
    # num_letters 
    # letter
    # password
    if len(string) != 0:
        match = re.match(REGEX_STRING, string)
        # Now just count the number of letters in the password
        letter_count = match.group('password').count(match.group('letter'))
        if int(letter_count) >= int(match.group('lower_bound')) and int(letter_count) <= int(match.group('upper_bound')):
            num_valid += 1
    else:
        break
